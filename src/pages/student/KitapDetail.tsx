...
const kitapData: Record<string, {
  ...
  sayfa: number
  aciklama: string
  bolumler?: { sayfa: number; baslik: string }[]
}> = {
  "kutlu-yolculuk": {
    id: "kutlu-yolculuk",
    ...
    bolumler: [
      { sayfa: 1, baslik: "Hicret Yolculuğu" },
    ],
  },
}
...
export function KitapDetailScreen() {
  const { kitapId } = useParams<{ kitapId: string }>()
  const k = kitapData[kitapId as keyof typeof kitapData]
  ...
  if (!k) {
    return (
      <div className="mx-auto max-w-md px-4 pt-16 text-center">
        <p className="text-sm text-muted-foreground">Kitap bulunamadı.</p>
        <Link to="/kitaplar" className="mt-4 inline-block text-sm font-medium text-primary">Geri Dön</Link>
      </div>
    )
  }

  const learned = isLearned("kitap", k.id)
    ...
    >
      ...
      <div className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-3 py-2.5">
          <Link to="/kitaplar" className="flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground">
            <ChevronLeft className="size-3.5" /> Kitaplar
          </Link>
          <div className="flex items-center gap-2">
            <span className="text-xl">{k.emoji}</span>
            <p className="text-sm font-bold text-foreground">{k.title}</p>
          </div>
          <Button variant="ghost" size="sm" onClick={() => learned ? unmarkLearned("kitap", k.id) : markLearned("kitap", k.id)} className="text-xs h-9 px-3 font-semibold">
            {learned ? "✓ Okundu" : "Okundu"}
          </Button>
        </div>

        {/* Bölüm sekmeleri */}
        {k.bolumler && k.bolumler.length > 0 && (
          <div className="mx-auto max-w-4xl overflow-x-auto pb-1 scrollbar-none">
            <div className="flex items-center gap-1 px-3">
              {k.bolumler.map((bolum, i) => {
                const isActive = currentPage >= bolum.sayfa && (i === k.bolumler!.length - 1 || currentPage < k.bolumler![i + 1].sayfa)
                return (
                  <button
                    key={i}
      ...
      >
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-2 px-3 py-2.5 flex-wrap sm:flex-nowrap">
          ...
          <div className="flex items-center gap-2 order-3 sm:order-none w-full sm:w-auto justify-center sm:justify-start mt-1.5 sm:mt-0">
            ...
            <input
              type="number" min={1} max={numPages || k.sayfa}
              value={currentPage}
              onChange={(e) => { const v = parseInt(e.target.value); if (v >= 1 && v <= (numPages || k.sayfa)) goToPage(v, v > currentPage ? 1 : -1) }}
            ...
            />
            <Button variant="outline" size="sm" onClick={handleNext} disabled={currentPage >= (numPages || k.sayfa)} className="h-10 rounded-lg text-xs font-bold gap-1 px-3">
              Sonraki <ChevronRight className="size-4" />
            </Button>
          </div>

          <span className="hidden sm:inline text-xs text-muted-foreground whitespace-nowrap">{k.subtitle}</span>
        </div>
      </div>
    </div>
  )
